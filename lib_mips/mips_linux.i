       ...
       ulong addr, ulong * len_ptr, int verify)
{
 ...
 theKernel =
  (void (*)(int, char **, char **, int)) ((__u32)(hdr->ih_ep));
# 102 "mips_linux.c"
 if (argc >= 3) {
  ;

  addr = simple_strtoul (argv[2], ((void *)0), 16);
 ...
 } else if ((hdr->ih_type == 4) && (len_ptr[1])) {
  ulong tail = ((__u32)(len_ptr[0])) % 4;
  int i;
 ...
 linux_params_init (((__typeof__((gd->bd->bi_boot_params)))(((unsigned long)((gd->bd->bi_boot_params)) & 0x1fffffff) | 0xa0000000)), commandline);
...
# 226 "mips_linux.c"
 ...
 sprintf (env_buf, "0x%08X", (uint) ((__typeof__((initrd_start)))(((unsigned long)((initrd_start)) & 0x1fffffff) | 0xa0000000)));
...
static void linux_params_init (ulong start, char *line)
{
 ...
 while (line && *line && linux_argc < 256) {
  quote = strchr (line, '"');
  next = strchr (line, ' ');

  while (next != ((void *)0) && quote != ((void *)0) && quote < next) {
...
# 314 "mips_linux.c"
 ...
 linux_env[0] = 0;
 linux_env_p = (char *) (linux_env + 256);
 linux_env_idx = 0;
}

static void linux_env_set (char *env_name, char *env_val)
{
 if (linux_env_idx < 256 - 1) {
