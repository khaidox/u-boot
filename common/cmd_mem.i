...
int do_mem_md ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
 ...
 if ((flag & 0x0001) == 0) {
  ...
  if (argc > 2)
   length = simple_strtoul(argv[2], ((void *)0), 16);
 }
...
int do_mem_mw ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
 ...
 writeval = simple_strtoul(argv[2], ((void *)0), 16);


 if (argc == 4) {
  count = simple_strtoul(argv[3], ((void *)0), 16);
 } else {
  count = 1;
 }
...
int do_mem_cp ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
 ...
 dest = simple_strtoul(argv[2], ((void *)0), 16);
 dest += base_address;
...
# 731 "cmd_mem.c"
 ...
 if (argc > 1) {
  start = (ulong *)simple_strtoul(argv[1], ((void *)0), 16);
 } else {
  start = (ulong *)0x80200000;
 }

 if (argc > 2) {
  end = (ulong *)simple_strtoul(argv[2], ((void *)0), 16);
 } else {
  end = (ulong *)(0x83800000);
 }

 if (argc > 3) {
  pattern = (ulong)simple_strtoul(argv[3], ((void *)0), 16);
 } else {
  pattern = 0;
 }
# 954 "cmd_mem.c"
...
cmd_tbl_t __u_boot_cmd_mm __attribute__ ((unused,section (".u_boot_cmd"))) = {"mm", 2, 1, do_mem_mm, "mm      - memory modify (auto-incrementing)\n", "[.b, .w, .l] address\n" "    - memory modify, auto increment address\n"}
...
cmd_tbl_t __u_boot_cmd_nm __attribute__ ((unused,section (".u_boot_cmd"))) = {"nm", 2, 1, do_mem_nm, "nm      - memory modify (constant address)\n", "[.b, .w, .l] address\n    - memory modify, read and keep address\n"}



 ;

cmd_tbl_t __u_boot_cmd_mw __attribute__ ((unused,section (".u_boot_cmd"))) = {"mw", 4, 1, do_mem_mw, "mw      - memory write (fill)\n", "[.b, .w, .l] address value [count]\n    - write memory\n"}



 ;

cmd_tbl_t __u_boot_cmd_mtest __attribute__ ((unused,section (".u_boot_cmd"))) = {"mtest", 4, 1, do_mem_mtest, "mtest   - simple RAM test\n", "[start [end [pattern]]]\n" "    - simple RAM read/write test\n"}
...
cmd_tbl_t __u_boot_cmd_cp __attribute__ ((unused,section (".u_boot_cmd"))) = {"cp", 4, 1, do_mem_cp, "cp      - memory copy\n", "[.b, .w, .l] source target count\n    - copy memory\n"}
