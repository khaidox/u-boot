...
# 46 "cmd_nvedit.c" 2
# 1 "/workspace/u-boot/include/serial.h" 1
...
# 32 "/workspace/u-boot/include/serial.h"
...
int do_printenv (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
 ...
 if (argc == 1) {
  ...
  printf("\nEnvironment size: %d/%d bytes\n", i, ((64*1024) - (sizeof(unsigned long))));
...
int _do_setenv (int flag, int argc, char *argv[])
{
 ...
 if (oldval >= 0) {
  ...
  if (strcmp(argv[1],"baudrate") == 0) {
   int baudrate = simple_strtoul(argv[2], ((void *)0), 10);
   int i;
   for (i=0; i<(sizeof(baudrate_table) / sizeof(baudrate_table[0])); ++i) {
    if (baudrate == baudrate_table[i])
     break;
   }
   if (i == (sizeof(baudrate_table) / sizeof(baudrate_table[0]))) {
 ...
 if (len > (&env_data[((64*1024) - (sizeof(unsigned long)))]-env)) {
 ...
 *++env = '\0';
 ...
 if (strcmp(argv[1],"ipaddr") == 0) {
  ...
  bd->bi_ip_addr = ((__u32)(addr));
  return 0;
 }
 if (strcmp(argv[1],"loadaddr") == 0) {
  load_addr = simple_strtoul(argv[2], ((void *)0), 16);
  return 0;
 }
...
{
 char *argv[4] = { "setenv", varname, varvalue, ((void *)0) };
 _do_setenv (0, 3, argv);
}
...
# 558 "cmd_nvedit.c"
static int
envmatch (uchar *$17, int i2)
{

 while (*$17 == env_get_char(i2++))
  if (*$17++ == '=')
   return(i2);
 if (*$17 == '\0' && env_get_char(i2-1) == '=')
  return(i2);
 return(-1);
}
...
cmd_tbl_t __u_boot_cmd_printenv __attribute__ ((unused,section (".u_boot_cmd"))) = {"printenv", 16, 1, do_printenv, "printenv- print environment variables\n", "\n    - print values of all environment variables\n" "printenv name ...\n" "    - print value of environment variable 'name'\n"}
...
cmd_tbl_t __u_boot_cmd_setenv __attribute__ ((unused,section (".u_boot_cmd"))) = {"setenv", 16, 0, do_setenv, "setenv  - set environment variables\n", "name value ...\n" "    - set environment variable 'name' to 'value ...'\n" "setenv name\n" "    - delete environment variable 'name'\n"}
...
cmd_tbl_t __u_boot_cmd_run __attribute__ ((unused,section (".u_boot_cmd"))) = {"run", 16, 1, do_run, "run     - run commands in an environment variable\n", "var [...]\n" "    - run the commands in the environment variable(s) 'var'\n"}
