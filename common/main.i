...
# 29 "main.c" 2
# 38 "main.c"
# 1 "../board/atheros/common/ath_flash.h" 1
# 37 "../board/atheros/common/ath_flash.h"
extern unsigned long flash_get_geom (flash_info_t *flash_info);
# 46 "../board/atheros/common/ath_flash.h"
...
# 1 "/workspace/u-boot/include/post.h" 1
# 42 "main.c" 2
# 55 "main.c"
...
{
 int abort = 0;
 uint64_t etime = (get_ticks() + (uint64_t)(bootdelay) * get_tbclk());
 struct
 {
  char* str;
  u_int len;
...
# 140 "main.c"
 ...
 for (i = 0; i < sizeof(delaykey) / sizeof(delaykey[0]); i ++) {
  delaykey[i].len = delaykey[i].str == ((void *)0) ?
        0 : strlen (delaykey[i].str);
  delaykey[i].len = delaykey[i].len > 32 ?
        32 : delaykey[i].len;
...
# 400 "main.c"
 s = getenv ("bootdelay");
 bootdelay = s ? (int)simple_strtol(s, ((void *)0), 10) : 1;
# 417 "main.c"
  s = getenv ("bootcmd");
       if (!s) {
# 427 "main.c"
...
# 478 "main.c"
   ...
   if (is_auto_upload_firmware)
   {
    ...
    strcpy(upload_cmd, "tftp 0x80800000 ");
    strcpy(image_name, "wr841nv9");
    strcat(image_name, "_tp_recovery.bin");
    ...
    memcpy(&original_product_id, (0x9F01F000 + 0xD00), 4);
    memcpy(&original_product_ver, (0x9F01F000 + 0xD04), 4);

    memcpy(&recovery_product_id, (0x80800000 + 0x40), 4);
    memcpy(&recovery_product_ver, (0x80800000 + 0x44), 4);
      ...
      && (original_product_ver== recovery_product_ver))
    {
     ...
     if (s)
     {
      file_size = simple_strtoul(s, ((void *)0), 16);
     }
...
{
 char c, prev;
 const char *varname_start = ((void *)0);
...
# 1065 "main.c"
 while (*str) {
  ...
  if ((cmdtp = find_cmd(argv[0])) == ((void *)0)) {
