...
void miiphy_init()
{
  do { (&mii_devs)->next = (&mii_devs); (&mii_devs)->prev = (&mii_devs); } while (0);
  current_mii = ((void *)0);
}
   ...
   unsigned char reg, unsigned short value))
{
 ...
 for (entry = (&mii_devs)->next, prefetch(entry->next); entry != (&mii_devs); entry = entry->next, prefetch(entry->next)) {
  miidev = ((struct mii_dev *)((char *)(entry)-(unsigned long)(&((struct mii_dev *)0)->link)));
 ...
 do { (&new_dev->link)->next = (&new_dev->link); (&new_dev->link)->prev = (&new_dev->link); } while (0);
 new_dev->read = read;
 new_dev->write = write;
...
int miiphy_set_current_dev(char *devname)
{
 ...
 for (entry = (&mii_devs)->next, prefetch(entry->next); entry != (&mii_devs); entry = entry->next, prefetch(entry->next)) {
  dev = ((struct mii_dev *)((char *)(entry)-(unsigned long)(&((struct mii_dev *)0)->link)));
   ...
   unsigned char *model, unsigned char *rev)
{
 ...
 if (miiphy_read (devname, addr, 0x03, &tmp) != 0) {
 ...
 if (miiphy_read (devname, addr, 0x02, &tmp) != 0) {
...
int miiphy_reset (char *devname, unsigned char addr)
{
 ...
 if (miiphy_read (devname, addr, 0x00, &reg) != 0) {



  return (-1);
 }
 if (miiphy_write (devname, addr, 0x00, reg | 0x8000) != 0) {
...
int miiphy_speed (char *devname, unsigned char addr)
{
 unsigned short reg;
# 370 "miiphyutil.c"
 if (miiphy_read (devname, addr, 0x00, &reg)) {
 ...
 if ((reg & 0x1000) != 0) {
  ...
  if (miiphy_read (devname, addr, 0x05, &reg)) {
...
int miiphy_link (char *devname, unsigned char addr)
{
 ...
 (void)miiphy_read(devname, addr, 0x01, &reg);
 if (miiphy_read (devname, addr, 0x01, &reg)) {
