 ...
 *p++ = '\0';
 ...
 if (ep == str || *ep != '\0' ||
  bank < 1 || bank > 1 ||
  ($30 = &flash_info[bank - 1])->flash_id == 0xFFFF)
  return -1;
 ...
 if (ep == str || *ep != '\0' || first >= $30->sector_count)
  return -1;

 if (p != ((void *)0)) {
  last = simple_strtoul (p, &ep, 10);
  if (ep == p || *ep != '\0' ||
   last < first || last >= $30->sector_count)
   return -1;
 } else {
  last = first;
 }
 ...
 *s_count = 0;
 ...
 for (bank=0,info=&flash_info[0];
      (bank < 1) && (addr_first <= addr_last);
      ++bank, ++info) {
  ulong b_end;
  int sect;
  short s_end;

  if (info->flash_id == 0xFFFF) {
   continue;
  }
...
# 375 "cmd_flash.c"
 ...
 if (strcmp(argv[1], "bank") == 0) {
  bank = simple_strtoul(argv[2], ((void *)0), 16);
  if ((bank < 1) || (bank > 1)) {
...
# 731 "cmd_flash.c"
cmd_tbl_t __u_boot_cmd_erase __attribute__ ((unused,section (".u_boot_cmd"))) = {"erase", 3, 1, do_flerase, "erase   - erase FLASH memory\n", "start end\n" "    - erase FLASH from addr 'start' to addr 'end'\n" "erase start +len\n" "    - erase FLASH from addr 'start' to the end of sect " "w/addr 'start'+'len'-1\n" "erase N:SF[-SL]\n    - erase sectors SF-SL in FLASH bank # N\n" "erase bank N\n    - erase FLASH bank # N\n" "erase all\n    - erase all FLASH banks\n"}
# 743 "cmd_flash.c"
 ;
