...
# 57 "tftp.c"
static char default_filename[(8 + 4 + 1)];
...
{
 ulong offset = block * 512 + TftpBlockWrapOffset;
...
TftpSend (void)
{
 ...
 pkt = NetTxPacket + NetEthHdrSize() + (sizeof (IP_t));
...
TftpHandler (uchar * pkt, unsigned dest, unsigned src, unsigned len)
{
 ...
 if (TftpState != 1 && src != TftpServerPort) {
  return;
 }
 ...
 case 3:
  ...
  if (TftpBlock == 0) {
   TftpBlockWrap++;
   TftpBlockWrapOffset += 512 * ((ulong)(1<<16));
   printf ("\n\t %lu MB reveived\n\t ", TftpBlockWrapOffset>>20);
  } else {
   if (((TftpBlock - 1) % 10) == 0) {
    putc ('#');
   } else if ((TftpBlock % (10 * 65)) == 0) {
    puts ("\n\t ");
   }
  }
  ...
  NetSetTimeout (2 * ((((((((((22) << 6) & 0x00000fc0)) & 0x00000fc0) >> 6)) * (40) * 1000000)) / 2), TftpTimeout);
  if(TftpTimeoutCount)
   TftpTimeoutCount = 0;
...
TftpTimeout (void)
{
 if (++TftpTimeoutCount > 3) {
  puts ("\nRetry count exceeded; starting again\n");


  if(fw_recovery) {
   load_addr = simple_strtoul("9f020000", ((void *)0), 16);
   do_bootm(((void *)0), 0, 0, ((void *)0));
  }
