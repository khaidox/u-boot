...
RarpTimeout(void)
{
 if (RarpTry >= 5) {
  ...
  NetSetTimeout (5 * ((((((((((22) << 6) & 0x00000fc0)) & 0x00000fc0) >> 6)) * (40) * 1000000)) / 2), RarpTimeout);
  RarpRequest ();
 }
}
...
RarpRequest (void)
{
 ...
 pkt += NetSetEther(pkt, NetBcastAddr, 0x8035);

 rarp = (ARP_t *)pkt;

 rarp->ar_hrd = ((__u16)(1));
 rarp->ar_pro = ((__u16)(0x0800));
 rarp->ar_hln = 6;
 rarp->ar_pln = 4;
 rarp->ar_op = ((__u16)(3));
 memcpy (&rarp->ar_data[0], NetOurEther, 6);
 memcpy (&rarp->ar_data[6], &NetOurIP, 4);
 ...
 NetSendPacket(NetTxPacket, (pkt - NetTxPacket) + (8+20));
