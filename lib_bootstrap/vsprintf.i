...
unsigned long simple_strtoul(const char *cp,char **endp,unsigned int base)
{
 ...
 if (*cp == '0') {
  cp++;
  if ((*cp == 'x') && (((_ctype[(int)(unsigned char)(cp[1])])&(0x04|0x40)) != 0)) {
   base = 16;
   cp++;
  }
  if (!base) {
 ...
 while ((((_ctype[(int)(unsigned char)(*cp)])&(0x04|0x40)) != 0) && (value = (((_ctype[(int)(unsigned char)(*cp)])&(0x04)) != 0) ? *cp-'0' : ((((_ctype[(int)(unsigned char)(*cp)])&(0x02)) != 0)
...

{
 ...
 if (num == 0)
  ...
  tmp[i++] = digits[({ int __res; __res = ((unsigned long) num) % (unsigned) base; num = ((unsigned long) num) / (unsigned) base; __res; })];
 if (i > precision)
  precision = i;
 size -= precision;
 if (!(type&(1 +16)))
