...
int lzma_inflate(unsigned char *source, int s_len, unsigned char *dest, int *d_len)
{
...

  {
    ...
    compressedSize = (SizeT)(length - (5 + 8));
  }
  ...
  if (LzmaDecodeProperties(&state.Properties, properties, 5) != 0)
  {
  ...
  state.Probs = (UInt16 *)malloc((1846 + (768 << ((&state.Properties)->lc + (&state.Properties)->lp))) * sizeof(UInt16));
