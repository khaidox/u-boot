...
Handle NewHandle (unsigned int numBytes)
{
 ...
 memPtr = malloc(numBytes*1);
 hanPtr = (HandleRecord *) malloc(sizeof (HandleRecord)*1);
...
{
 if (numNewItems == 0) {
  numNewItems = (((((*list)->listSize * ((*list)->percentIncrease + 100)) / 100)>((*list)->minNumItemsIncrease)) ? (((*list)->listSize * ((*list)->percentIncrease + 100)) / 100) : ((*list)->minNumItemsIncrease));
 } else if (numNewItems < 0) {
  numNewItems = (-numNewItems) + (((((*list)->listSize * ((*list)->percentIncrease + 100)) / 100)>((*list)->minNumItemsIncrease)) ? (((*list)->listSize * ((*list)->percentIncrease + 100)) / 100) : ((*list)->minNumItemsIncrease));
 }
...
list_t ListCreate (int elementSize)
{
 ...
 if (list) {
  (*list)->signature = (('L'<<24) | ('I'<<16) | ('S'<<8) | 'T');;
  (*list)->numItems = 0;
  (*list)->listSize = 0;
  (*list)->itemSize = elementSize;
  (*list)->percentIncrease = 10;
       ...
       int numItemsToInsert)
{
 ...
 if (firstItemPosition == 0) {
  if (ptrToItems)
   memcpy (&(((char *)&(*list)->itemList)[(*(list))->itemSize * (numItems)]), ptrToItems,
     (*list)->itemSize * numItemsToInsert);
  else
   memset (&(((char *)&(*list)->itemList)[(*(list))->itemSize * (numItems)]), 0,
     (*list)->itemSize * numItemsToInsert);

  (*list)->numItems += numItemsToInsert;
 } else {
  memmove (&(((char *)&(*list)->itemList)[(*(list))->itemSize * (firstItemPosition - 1 + numItemsToInsert)]),
    &(((char *)&(*list)->itemList)[(*(list))->itemSize * (firstItemPosition - 1)]),
    (numItems + 1 - firstItemPosition) * (*list)->itemSize);

  if (ptrToItems)
   memmove (&(((char *)&(*list)->itemList)[(*(list))->itemSize * (firstItemPosition - 1)]), ptrToItems,
      (*list)->itemSize * numItemsToInsert);
  else
   memset (&(((char *)&(*list)->itemList)[(*(list))->itemSize * (firstItemPosition - 1)]), 0,
     (*list)->itemSize * numItemsToInsert);
...
void *ListGetPtrToItem (list_t list, int itemPosition)
{
 ...
 return &(((char *)&(*list)->itemList)[(*(list))->itemSize * (itemPosition - 1)]);
}
