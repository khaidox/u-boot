# 1 "../common/athr_s27_phy.c"
# 1 "/workspace/u-boot/board/atheros/board953x//"
# 1 "<command-line>"
# 1 "../common/athr_s27_phy.c"
# 16 "../common/athr_s27_phy.c"
...
# 19 "../common/athr_s27_phy.c" 2
# 1 "/workspace/u-boot/include/miiphy.h" 1
# 41 "/workspace/u-boot/include/miiphy.h"
...
# 42 "/workspace/u-boot/include/miiphy.h" 2
...
# 21 "../common/athr_s27_phy.c" 2


# 1 "../common/athr_s27_phy.h" 1
# 175 "../common/athr_s27_phy.h"
typedef enum {
    NORMAL_PACKET,
    RESERVED0,
    MIB_1ST,
...
{
    s27_wr_phy(phy_addr,29,0x29);
    s27_wr_phy(phy_addr,30,0x36c0);

}
void athrs27_sleep_off(int phy_addr)
{
    s27_wr_phy(phy_addr,29,0xb);
    s27_wr_phy(phy_addr,30,0x3c00);
}
...
void athrs27_force_100M(int phyAddr,int duplex)
{
    ...
    if(phyAddr%2) {
        s27_wr_phy(phyAddr,16,0x820);
    }
    else {
        s27_wr_phy(phyAddr,16,0x800);
    }

    s27_wr_phy(phyAddr,0x1d,0x29);
    s27_wr_phy(phyAddr,0x1e,0x0);
    s27_wr_phy(phyAddr,0x10,0xc60);
    s27_wr_phy(phyAddr,0,(0xa000|(duplex << 8)));
}
...
void athrs27_force_10M(int phyAddr,int duplex)
{
    ...
    s27_wr_phy(phyAddr,0,(0x8000 |(duplex << 8)));
}
...
int athrs27_reg_init(void)
{
    ...
    s27_wr_phy(0x4,0,0x9000);
# 228 "../common/athr_s27_phy.c"
    return 0;
}
...
int athrs27_reg_init_lan(void)
{
    ...
    printf("ATHRS27" ": s27 reset done\n");
    athrs27_reg_write(0x0100,0x4e);

    athrs27_reg_rmw(0x4,(1<<6));
    ...
    for (phyUnit=0; phyUnit < 5; phyUnit++) {
        ...
        foundPhy = 1;
        phyBase = (athrPhyInfo[phyUnit].phyBase);
        phyAddr = (athrPhyInfo[phyUnit].phyAddr);
        ...
        s27_wr_phy(phyAddr,0,0x9000);
# 285 "../common/athr_s27_phy.c"
    }
# 295 "../common/athr_s27_phy.c"
    athrs27_reg_write(0x0200, 0x200);
    athrs27_reg_write(0x0300, 0x200);
    athrs27_reg_write(0x0400, 0x200);
    athrs27_reg_write(0x0500, 0x200);

    if ((0)) {
        athrs27_reg_write(0x0200, 0x4C);
        athrs27_reg_write(0x0300, 0x4c);
        athrs27_reg_write(0x0400, 0x4c);
        athrs27_reg_write(0x0500, 0x4c);
    }


    athrs27_reg_write(0x38, 0xc000050e);
# 319 "../common/athr_s27_phy.c"
    athrs27_reg_write(0x0104, 0x4004);
    ...
    athrs27_reg_write(0x005c,(athrs27_reg_read(0x005c) | 0x100000));


    athrs27_reg_write(0x002c,(athrs27_reg_read(0x002c) |
                           (1 << 26) | (1 << 25) ));

    return 0;
}
# 343 "../common/athr_s27_phy.c"
int
athrs27_phy_is_link_alive(int phyUnit)
{
    ...
    phyBase = (athrPhyInfo[phyUnit].phyBase);
    phyAddr = (athrPhyInfo[phyUnit].phyAddr);
    phyHwStatus = s27_rd_phy(phyAddr, 17);

    if (phyHwStatus & 0x0400)
        return 1;

    return 0;
}
# 371 "../common/athr_s27_phy.c"
int
athrs27_phy_setup(int ethUnit)
{
    ...
    for (phyUnit=0; phyUnit < 5; phyUnit++) {
        ...
        if (!((athrPhyInfo[phyUnit].isEnetPort) && (athrPhyInfo[phyUnit].ethUnit) == (ethUnit))) {
            continue;
        }
        if (!(0)) {
           s27_wr_phy(phyAddr, 4,(0x0800 | 0x0400 | 0x0020 | 0x0040 | 0x0080 | 0x0100));

           s27_wr_phy(phyAddr, 0,0x1000
                         | 0x8000);
        }
        else {
  printf("############ is emulation ############\n");

           if((athrPhyInfo[phyUnit].ethUnit) == 0) {
               s27_wr_phy(phyAddr, 4,(0x0800 | 0x0400 | 0x0020 | 0x0040 | 0x0080 | 0x0100));
               s27_wr_phy(phyAddr,0x9, 0x0);
               s27_wr_phy(phyAddr, 0,0x0);
               s27_wr_phy(phyAddr, 0,0x1000
                         | 0x8000);
           }
           else {

               s27_wr_phy(phyAddr, 4,(0x0800 | 0x0400 |
                            0x0020 | 0x0040));
               s27_wr_phy(phyAddr,0x9, 0x0);
               s27_wr_phy(phyAddr, 0,0x0);
               s27_wr_phy(phyAddr, 0,0x1000
       ...
       rd_val = s27_rd_phy(phyAddr,0);
       printf("%s ATHR_PHY_CONTROL %d:0x%x\n",__func__,phyAddr,rd_val);
       rd_val = s27_rd_phy(phyAddr,17);
    ...
    for (phyUnit=0; (phyUnit < 5) ; phyUnit++) {
        ...
        timeout=20;
        for (;;) {
            phyHwStatus = s27_rd_phy(phyAddr, 0);

            if ((((phyHwStatus) & (0x8000)) == 0)) {
        ...
        s27_wr_phy(phyUnit, 30, 0xf52);


        s27_wr_phy(phyUnit, 29, 4);
        s27_wr_phy(phyUnit, 30, 0xebbb);
        s27_wr_phy(phyUnit, 29, 5);
        s27_wr_phy(phyUnit, 30, 0x2c47);


        s27_wr_phy(phyUnit, 29, 0x3c);
        s27_wr_phy(phyUnit, 30, 0x1c1);
        s27_wr_phy(phyUnit, 29, 0x37);
        s27_wr_phy(phyUnit, 30, 0xd600);
# 496 "../common/athr_s27_phy.c"
    }
# 507 "../common/athr_s27_phy.c"
    for (phyUnit=0; phyUnit < 5; phyUnit++) {
        if (!((athrPhyInfo[phyUnit].isEnetPort) && (athrPhyInfo[phyUnit].ethUnit) == (ethUnit))) {
            continue;
        }
        ...
        if (athrs27_phy_is_link_alive(phyUnit)) {
            liveLinks++;
            (athrPhyInfo[phyUnit].isPhyAlive) = 1;
        } else {
            (athrPhyInfo[phyUnit].isPhyAlive) = 0;
        }
...
athrs27_phy_is_fdx(int ethUnit,int phyUnit)
{
    ...
    for (phyUnit=0; phyUnit < 5; phyUnit++) {
        ...
        if (athrs27_phy_is_link_alive(phyUnit)) {
            ...
            do {
                phyHwStatus = s27_rd_phy (phyAddr, 17);
                        if(phyHwStatus & 0x0800)
                                break;
                udelay((10) * 1000);
            } while(--ii);
            if (phyHwStatus & 0x2000) {
                return 1;
            }
        }
    }
...
athrs27_phy_speed(int ethUnit,int phyUnit)
{
    ...
    for (phyUnit=0; phyUnit < 5; phyUnit++) {
        ...
        if (athrs27_phy_is_link_alive(phyUnit)) {
            ...
            phyHwStatus = ((phyHwStatus & 0xC000) >>
                           14);
  ...
  break;
        ...
        ath_gmac_miiphy_write((1 ? "eth1" : "eth0"), phyAddr, 29, 0x18);

        if(phySpeed == 100) {
            ath_gmac_miiphy_write((1 ? "eth1" : "eth0"), phyAddr, 30, 0xba8);
        } else {
            ath_gmac_miiphy_write((1 ? "eth1" : "eth0"), phyAddr, 30, 0x2ea);
        }
    }
    ...
    for (phyUnit=0; phyUnit < 5; phyUnit++) {
        ...
        if (lastStatus->isPhyAlive) {
            ...
            if (phyHwStatus & 0x0400) {
                ...
                lastStatus->isPhyAlive = 0;
            }
        } else {

            if((0))
            {
                phyHwStatus = s27_rd_phy(phyAddr, 1);
                if(phyAddr%2) {
                    s27_wr_phy(phyAddr,16,0x820);
                }
                else {
            ...
            if (!(((phyHwStatus) & (0x8000)) == 0))
             ...
             phyHwControl = s27_rd_phy(phyAddr, 0);
             phyHwStatus = s27_rd_phy(phyAddr, 1);


            if ((!(phyHwControl & 0x1000))
                 || (((phyHwStatus) & (0x0020)) == (0x0020))) {
                phyHwStatus = s27_rd_phy(phyAddr,
                                           17);
...
unsigned int athrs27_reg_read(unsigned int s27_addr)
{
    ...
    ath_gmac_miiphy_write((unit ? "eth1" : "eth0"), phy_address, reg_address, data);

    phy_address = (0x17 & ((addr_temp >> 4) | 0x10));
    reg_address = ((addr_temp << 1) & 0x1e);
    s27_rd_csr_low = (uint32_t) ath_gmac_miiphy_read((unit ? "eth1" : "eth0"), phy_address, reg_address);

    reg_address = reg_address | 0x1;
    s27_rd_csr_high = (uint32_t) ath_gmac_miiphy_read((unit ? "eth1" : "eth0"), phy_address, reg_address);
